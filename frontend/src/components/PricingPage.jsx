...
import { Check, X, HelpCircle, Minus, Plus, ArrowRight } from 'lucide-react';
...
const PricingCard = ({ plan, isYearly }) => {
  ...
  return (
    <div className={`relative flex flex-col p-8 rounded-2xl transition-all duration-300 ${
      plan.popular 
        ? 'bg-white border-2 border-teal-500 shadow-xl scale-105 z-10' 
        : 'bg-white border border-slate-200 shadow-sm hover:shadow-lg hover:border-teal-200'
    }`}>
      {plan.popular && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-teal-600 text-white px-4 py-1 rounded-full text-sm font-semibold shadow-sm tracking-wide">
          MOST POPULAR
        </div>
      )}

      <div className="mb-6">
        <h3 className={`text-xl font-bold ${plan.popular ? 'text-teal-700' : 'text-slate-900'}`}>{plan.name}</h3>
      ...
      <div className="mb-8">
        <div className="flex items-baseline">
          ...
          <span className="text-slate-500 ml-2 font-medium">{price === 0 ? '' : period}</span>
        </div>
        <p className="text-slate-400 text-xs mt-1 font-medium">{price === 0 ? 'Forever free' : subtext}</p>
      </div>
            ...
            <li key={idx} className="flex items-start">
              {feature.included ? (
                <div className="flex-shrink-0 w-5 h-5 rounded-full bg-teal-100 flex items-center justify-center mt-0.5">
                  <Check size={12} className="text-teal-700" strokeWidth={3} />
                </div>
              ) : (
              ...
              <span className={`ml-3 text-sm ${feature.included ? 'text-slate-600' : 'text-slate-400 line-through'}`}>
                {feature.text}
              </span>
            </li>
          ))}
...
const FaqItem = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-slate-200 last:border-0">
      ...
      >
        <span className={`text-lg font-medium transition-colors ${isOpen ? 'text-teal-700' : 'text-slate-900 group-hover:text-teal-700'}`}>
          {question}
        </span>
        <span className={`ml-6 flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>
          {isOpen ? <Minus size={20} className="text-teal-600" /> : <Plus size={20} className="text-slate-400" />}
        </span>
      </button>
      <div 
  ...
  const faqs = [
    {
      ...
      answer: "Absolutely. You can downgrade or cancel your plan at any time from your dashboard. If you cancel, you'll still have access to your premium features until the end of your billing cycle."
    },
    {
      question: "What happens if I go over my response limit?",
      answer: "On the Free plan, we'll notify you when you approach the limit. We won't close your surveys immediately, but you won't be able to view new responses until the next month or unless you upgrade."
    },
    {
      question: "Do you offer a discount for non-profits?",
      answer: "Yes! We love supporting organizations that do good. Contact our support team with proof of your non-profit status and we'll set you up with a 20% discount on all paid plans."
    },
    {
      question: "Is my data secure?",
      answer: "Security is our top priority. We use industry-standard encryption for data in transit and at rest. Our Harmony plan also offers advanced security features like SSO."
    }
  ];
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <main>
        ...
        <div className="relative pt-12 pb-20 sm:pt-16 sm:pb-32 overflow-hidden">
           ...
           <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-teal-50 rounded-[100%] blur-3xl -z-10 opacity-60 pointer-events-none"></div>

          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 mb-6">
              Simple pricing for <span className="text-teal-600">mindful</span> data.
            </h1>
            ...
            <div className="flex items-center justify-center gap-4 mb-16">
              ...
              <span className={`text-sm font-medium ${isYearly ? 'text-slate-900' : 'text-slate-500'}`}>
                Yearly <span className="text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full text-xs ml-1 font-bold">SAVE 20%</span>
              </span>
            </div>
        ...
        <div className="bg-slate-50 py-24 border-y border-slate-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-12 opacity-50 grayscale">
               ...
               <div className="flex items-center gap-2 text-xl font-bold text-slate-800"><div className="w-6 h-6 bg-slate-400 rounded-full"></div>Acme Corp</div>
               <div className="flex items-center gap-2 text-xl font-bold text-slate-800"><div className="w-6 h-6 bg-slate-400 rounded-md"></div>GlobalBank</div>
               <div className="flex items-center gap-2 text-xl font-bold text-slate-800"><div className="w-6 h-6 bg-slate-400 rotate-45"></div>Nebula</div>
               <div className="flex items-center gap-2 text-xl font-bold text-slate-800"><div className="w-6 h-6 bg-slate-400 rounded-sm"></div>FoxRun</div>
            </div>
          </div>
        </div>
        ...
        <div className="py-24 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 text-center bg-teal-50 rounded-2xl p-8">
            <p className="font-medium text-teal-900 mb-4">Still have questions?</p>
            <p className="text-teal-700 mb-6 text-sm">Our team is happy to answer your questions. We usually reply within a few hours.</p>
            <button className="bg-teal-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-teal-700 transition-colors flex items-center gap-2 mx-auto">
              <HelpCircle size={18} />
              Contact Support
            </button>
          </div>
        ...
        <div className="py-20 bg-slate-900 text-white text-center">
            <div className="max-w-4xl mx-auto px-6">
              <h2 className="text-3xl sm:text-4xl font-bold mb-6">Ready to create your first Zen survey?</h2>
              <p className="text-slate-300 mb-8 text-lg">Join 10,000+ creators collecting better data today.</p>
              <button className="bg-teal-500 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-teal-400 transition-all shadow-lg hover:shadow-teal-500/25 flex items-center gap-2 mx-auto">
